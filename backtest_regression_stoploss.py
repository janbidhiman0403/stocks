...
while i < len(results):
    ...
    if row["Predicted_Return_5D"] >= cutoff:
        ...
        if exit_index < len(results):
            ...
            # Simplified closing-price stop-loss
            trade_return = max(
                actual_return,
                stop_loss
            )
...
if len(trades_df) > 0:
    ...
    print(
        ...
        round(
            trades_df["Strategy_Return"].mean() * 100,
            2
        ),
        "%"
    )
    ...
    print(
        "Best Trade:",
        round(
            trades_df["Strategy_Return"].max() * 100,
            2
        ),
        "%"
    )
    ...
    print(
        "Worst Trade:",
        round(
            trades_df["Strategy_Return"].min() * 100,
            2
        ),
        "%"
    )
    ...
    trades_df.to_csv(
        "data/regression_stoploss_trades.csv",
        index=False
    )

    print(
        "\nSaved to: data/regression_stoploss_trades.csv"
    )